@extends('layouts.app', ['activePage' => 'Variation', 'titlePage' => __('Edit Variation')])
...
@section('content')
    <div class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <form method="post" action="{{ route('variation.update', $variation->id) }}" autocomplete="off" class="form-horizontal" enctype="multipart/form-data">
                        @csrf
                        @method('put')

                        <div class="card ">
                            <div class="card-header card-header-primary">
                                <h4 class="card-title">{{ __('Update Variation') }}</h4>
                                    ...
                                    <div class="col-sm-7">
                                        <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                            <input
                                                ...
                                                placeholder="{{ __('Variation Name') }}" value="{{ old('name', $variation->name) }}"
                                    ...
                                    <div class="col-sm-7">
                                        <div class="form-group{{ $errors->has('slug_name') ? ' has-danger' : '' }}">
                                            <input
                                                ...
                                                placeholder="{{ __('Slug Name') }}" value="{{ old('slug_name', $variation->slug_name) }}"
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-7">
                                        <div class="form-group{{ $errors->has('Variation Attribute') ? ' has-danger' : '' }}">
                                            @php
                                                $selectedOptions = $variation->variation_option_values()->orderBy('name')->pluck('variation_option_id')->toArray();
                                            @endphp
                                            <select
                                                ...
                                                @foreach($variationOptions as $variationOption)
                                                    <option data-value="{{ $variationOption->name }}" value="{{ $variationOption->name }}" @if(in_array($variationOption->id, $selectedOptions)) selected @endif>{{ $variationOption->name }}</option>
                                                @endforeach
                                    ...
                                    <div class="col-sm-7">
                                        <div class="form-group{{ $errors->has('status') ? ' has-danger' : '' }}">
                                            <input style="width: 62px;"
                                                ...
                                                @if(old('status', $variation->status) == 1) checked @endif
                                                aria-required="true" />
                            ...
                            <div class="card-footer ml-auto mr-auto">
                                <a href="{{ route('variation.index') }}" class="btn btn-info">{{ __('Cancel') }}</a>
